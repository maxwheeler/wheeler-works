...
description: "Budget by Mail turns the transaction-alert emails your bank already sends into a categorized budget — automatically, inside a Google Sheet you own. There are no bank logins and no account linking: you point a few Gmail filters at the alert emails you already receive, and Budget by Mail reads each one, uses AI to pull out the merchant, amount, and account, categorizes it, and appends it to your spreadsheet. Formula-driven dashboards — monthly, annual, cashflow, and trends — then update themselves as transactions land. It runs entirely in your own Google account as a Google Workspace add-on (nothing to install on your computer, no third-party service holding your history); fix a category once and it remembers that merchant forever; and new users get AI categorization out of the box with no API key required. It's invitation-only during early access — request access at budgetbymail.com."
